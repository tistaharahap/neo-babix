class WilliamsIndicators(object):
    ...
    def SMMA(self, highs, lows, n_smoothing_periods, future_shift):
        '''
        ...
        if len(highs) < n_smoothing_periods:
            print('too short')
            return
        ...
        for i in range(len(highs)):
            median_price = (float(highs[i]) + float(lows[i])) / 2  # (high + low) / 2
        ...
        for i in range(n_smoothing_periods, len(highs) + future_shift):
    ...
    def check_bar_type(self, two_candles_list):
        ...
        if float(two_candles_list[1][1]) < float(two_candles_list[1][3]) + one_third:
            bar_type += '1'
        elif float(two_candles_list[1][3]) + 2 * one_third < float(two_candles_list[1][1]) < float(
                two_candles_list[1][3]) + 3 * one_third:
            bar_type += '3'
        else:
            bar_type += '2'
        ...
        if float(two_candles_list[1][4]) < float(two_candles_list[1][3]) + one_third:
            bar_type += '1'
        elif float(two_candles_list[1][3]) + 2 * one_third < float(two_candles_list[1][4]) < float(
                two_candles_list[1][3]) + 3 * one_third:
            bar_type += '3'
        else:
            bar_type += '2'
